...
def YOLO():
    ...
    while True:
        ...
        if (frame_read is None):
            break
        #비디오 받아서 차선 인식 후 차선이 그려진 이미지 획득
        #차선이 그려진 이미지 = laneImage
        laneImage = ld.lane_detection_and_draw(frame_read)
        #차선이 그려진 이미지 리사이즈 = laneImageResize
        laneImageResize = cv2.resize(laneImage,
        ...
        image = cvDrawBoxes(detections, laneImageResize)
